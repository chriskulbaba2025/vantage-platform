...
**Supersedes:** PRYSM_REBUILD_ACCELERATION_STANDARD_v1.0.md (retained as historical reference)  
**Applies to:** Every Prysm rebuild work package, correction, review, acceptance gate, and release decision from WP7 onward
...
```text
Read and obey PRYSM_REBUILD_ACCELERATION_STANDARD_v1.1.md before editing.
...
## 15. v1.1 — Proof-only correction automation (NEW)

### 15.1 Production correction limit

Maximum one consolidated correction round that changes governed production behaviour.

A second production-behaviour correction requires process review.

### 15.2 Proof-only corrections

Permit up to two automatic proof-only correction rounds without owner intervention.

Proof-only changes may modify only:

- checklist proof details;
- unit tests;
- acceptance tests;
- fixtures;
- mocks;
...
They may not change governed production behaviour.

### 15.3 Testability-seam corrections

A correction may be classified proof-only when ALL are true:

- production default behaviour is unchanged;
- the seam only injects deterministic controlled dependencies;
- no new client-facing behaviour;
- no security boundary weakened;
- no new live dependency;
- regression proof confirms default production behaviour unchanged.
...
### 15.4 Patch-level checklist updates

Allow automatic patch-level checklist updates when they only:

- add an omitted test/proof file;
- add an acceptance fixture;
- add a testability seam required for an existing ID;
- clarify direct proof for an existing requirement.
...
### 15.5 Test-file scope rule

A new test file whose sole purpose is proving existing frozen checklist IDs may automatically be added to permitted scope through a patch-level checklist update.

### 15.6 Automatic proof correction loop

When independent audit failures are exclusively proof/testability/scope-bookkeeping defects:

1. classify failed IDs;
2. apply bounded proof-only correction;
3. verify;
4. commit/push;
5. rerun exact-head CI;
6. rerun fresh independent audit;
7. repeat one additional proof-only round if necessary.

Do not stop for owner approval between those proof-only rounds.

### 15.7 Hard stops remain

Still stop for:

- second production-behaviour correction;
- material architecture change;
- report/golden-master change;
- lifecycle/security/evidence-contract change;
- live or paid provider call;
- live or paid LLM call;
- deployment;
- production environment modification;
- credentials;
- IAM;
- destructive Git;
- merge of an implementation WP;
- material new WP scope.

### 15.8 Git repository autonomy

After scope verification, governance permits:

- git status
- git diff
- git add <explicit permitted files>
- git commit
...
- force push
- branch deletion
- unapproved implementation-WP merge

Do not attempt to bypass or disable Claude Code platform safety.

If Claude Code itself blocks an otherwise governed Git command:

- retry through a normal permitted repository mechanism once;
- if still blocked, report only that blocked command;
- do not redo completed implementation work.

### 15.9 Audit quality unchanged

- exact-head independent audit remains mandatory;
- green CI alone is insufficient;
- every checklist ID requires direct executable proof;
- correction allowances cannot weaken PASS criteria.

### 15.10 Report lock unchanged

All report/template/golden-master protections remain exactly intact.
...
Read:
- PRYSM_REBUILD_ACCELERATION_STANDARD_v1.1.md
...
- Added §15.1: Production correction limit — max one consolidated production-behaviour correction round.
- Added §15.2: Proof-only corrections — up to two automatic proof-only correction rounds.
- Added §15.3: Testability-seam correction classification criteria.
- Added §15.4: Patch-level checklist update rules.
- Added §15.5: Test-file scope rule for adding proof-only test files.
- Added §15.6: Automatic proof correction loop procedure.
- Added §15.7: Enumerated hard stops (unchanged from v1.0, explicitly restated).
- Added §15.8: Git repository autonomy rules.
- Added §15.9: Audit quality unchanged affirmation.
- Added §15.10: Report lock unchanged affirmation.
- Updated all template references from v1.0 to v1.1.
- v1.0.0 retained as historical reference; all v1.0 governance preserved.
