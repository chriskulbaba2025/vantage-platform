...
- [x] UI-01 — Existing v2 governed artifact remains `report-v2/pages/index.html`; no server/routing/storage changes.
- [x] UI-02 — Exactly 15 left-nav entries match the governed PRD section order.
- [x] UI-03 — Desktop navigation is sticky and independently scrollable.
- [x] UI-04 — Normal view displays only the selected conceptual report page; existing generated sections are not deleted or rewritten.
- [x] UI-05 — URL hash selects the conceptual page; invalid/missing hash safely falls back to Executive Scorecard.
- [x] UI-06 — Active navigation state is exposed with `aria-current="page"` and updated deterministically.
- [x] UI-07 — Each conceptual page exposes a visible `Print or save this page as PDF` browser-print control.
- [x] UI-08 — Print CSS hides navigation, controls, and every non-active conceptual page; only the selected page is printable.
- [x] UI-09 — Responsive layout preserves the approved left-hand navigation, narrowing the rail at smaller viewport widths.
- [x] UI-10 — Existing report section generators remain in their current order and consume the same governed model.
- [x] UI-11 — v1 renderer remains unchanged.
- [x] UI-12 — Report presentation identifies viewer/design presentation version `2.1.0` while preserving the existing v2.0.0 audit selector/data contract.
- [x] UI-13 — Deterministic render regression remains byte-identical for identical inputs.
- [x] UI-14 — Tests prove all 15 page keys, active-page logic hooks, print isolation rules, and responsive left-navigation markup.
...
- [x] UI-17 — Before/after visual review completed against the approved standalone HTML mockup; accessible navigation labels and current-page state are retained.
- [x] UI-18 — Principal Auditor/owner explicitly approved the left-sidebar design and authorized release on 2026-08-21.
