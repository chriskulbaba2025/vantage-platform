...
export function validateWriterSemanticFidelity(
  ...
  const boundedAiPattern =
    /\b(?:may|might|could|potential|opportunity|not assessed|not established|does not establish|does not (?:directly )?assess(?:ed|ment)?[^.!?]{0,80}\b(?:to )?establish|cannot (?:be )?(?:evaluated|assessed|established|concluded|determined)|requires? (?:separate )?assessment)\b/i;
