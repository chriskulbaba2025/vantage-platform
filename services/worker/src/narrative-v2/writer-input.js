...
function copyOwn(source, fields) {
  ...
  for (const field of fields) {
    ...
    out[field] = cloneDefined(source[field]);
  }
  return out;
}
...
function projectCapabilities(capabilityEvidence, auditId) {
  ...
  const capabilities = {};
  for (const [key, record] of Object.entries(capabilityEvidence.capabilities)) {
    if (!record || typeof record !== "object" || Array.isArray(record)) {
      throw new Error(`Capability ${key} must be an object`);
    }
    for (const field of CAPABILITY_REQUIRED_FIELDS) {
      if (!Object.hasOwn(record, field) || record[field] === undefined) {
        throw new Error(`Capability ${key} missing canonical field: ${field}`);
      }
    }
    if (record.capability !== key) {
      throw new Error(`Capability key mismatch: ${key} vs ${record.capability}`);
    }

    const projected = copyOwn(record, [
    ...
    capabilities[key] = Object.freeze(projected);
  }

  const result = { capabilities: Object.freeze(capabilities) };
  for (const field of ["capabilityEvidenceVersion", "generatedAt", "summary"]) {
    if (!Object.hasOwn(capabilityEvidence, field) || capabilityEvidence[field] === undefined) continue;
    result[field] = cloneDefined(capabilityEvidence[field]);
  }
...
function assertFindingIntegrity(scoreSet, findings) {
  ...
  if (new Set(scoreIds).size !== scoreIds.length) throw new Error("scoreSet.findingIds contains duplicates");
  if (new Set(findingIds).size !== findingIds.length) throw new Error("findings contains duplicate findingId values");

  const scoreSetIds = new Set(scoreIds);
  const findingSetIds = new Set(findingIds);
  const missing = scoreIds.filter((id) => !findingSetIds.has(id));
  const unexpected = findingIds.filter((id) => !scoreSetIds.has(id));
  if (missing.length || unexpected.length) {
    throw new Error(`FindingSet does not match ScoreSet findingIds; missing=[${missing.join(",")}], unexpected=[${unexpected.join(",")}]`);
  }
}
