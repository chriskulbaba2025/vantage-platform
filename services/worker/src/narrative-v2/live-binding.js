...
export function createNarrativeV2LiveBinding({
  ...
  const finalPassAuthorizations = new Map();
  ...
  function registerAuditScope({
    ...
    scopes.set(
      ...
      Object.freeze({
        ...
        auditId,
        executionId:
          executionId || `${auditId}:narrative-v2`,
      }),
    );
  }
  ...
  function authorizeFinalPass({
    ...
    if (finalPassAuthorizations.has(auditId)) {
      throw new Error(
    ...
    finalPassAuthorizations.set(auditId, authorization);
  ...
  async function existingReservations(scope) {
    ...
    for (
      let i = 1;
      i <= NARRATIVE_V2_LIVE_MAX_TOTAL_CALLS;
      i += 1
    ) {
  ...
  async function requireValidatedResult(scope, reservation, label) {
    ...
    try {
      ...
      if (
        passNumber <= NARRATIVE_V2_LIVE_MAX_AUTOMATIC_PASSES
        && existing.length >= NARRATIVE_V2_LIVE_MAX_CALLS
      ) {
        throw new Error(
      ...
      if (existing.length >= NARRATIVE_V2_LIVE_MAX_TOTAL_CALLS) {
        throw new Error(
      ...
      if (role === "writer" && passNumber === 1) {
        ...
        if (
          previousJudgeResponse?.decision !== JUDGE_DECISION.REVISE
      ...
      } else if (role === "writer" && passNumber === 3) {
        const ordered =
          existing.length === 4
          && existing[0].role === "writer"
          && existing[0].passNumber === 1
        ...
        finalPassAuthorization =
          finalPassAuthorizations.get(scope.auditId) || null;
      ...
      } else if (role === "judge" && passNumber === 3) {
        const ordered =
          existing.length === 5
          && existing[0].role === "writer"
          && existing[0].passNumber === 1
  ...
  Object.defineProperty(writerExecutor, "maxAutomaticPasses", {
    ...
    config: Object.freeze({
      ...
      maxTotalCallsPerAudit: NARRATIVE_V2_LIVE_MAX_TOTAL_CALLS,
