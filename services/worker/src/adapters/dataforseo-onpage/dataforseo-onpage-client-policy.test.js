...
test("representative On-Page task payload carries the governed acquisition policy", withCredentials(async () => {
  const calls = [];
  const priorityUrls = [
    "https://example.com/",
    ...Array.from(
      { length: 25 },
      (_, index) => `https://example.com/locations/city-${index + 1}`,
    ),
    "https://example.com/",
  ];
  ...
  assert.equal(payload.max_crawl_pages, 500);
  ...
  assert.equal(payload.priority_urls[0], "https://example.com/");
  assert.equal(new Set(payload.priority_urls).size, 20);
}));

test("task_post transport failure is not automatically reposted", withCredentials(async () => {
  let calls = 0;
...
test("main On-Page polling policy is 30 minutes", () => {
  assert.equal(
