...
test("PRYSM-V2-SECTION-VIEWER-01: viewer is versioned and has exactly 15 governed pages", () => {
  assert.equal(REPORT_V2_VIEWER_VERSION, "2.1.0");
  assert.equal(REPORT_V2_VIEWER_PAGES.length, 15);
  assert.deepEqual(REPORT_V2_VIEWER_PAGES.map((page) => page.title), EXPECTED_PAGES);
});

test("PRYSM-V2-SECTION-VIEWER-01: every existing top-level report section is assigned once", () => {
  const sectionIds = REPORT_V2_VIEWER_PAGES.flatMap((page) => [...page.sectionIds]);
  assert.equal(sectionIds.length, EXPECTED_SECTION_IDS.length);
  assert.equal(new Set(sectionIds).size, sectionIds.length, "no report section may be assigned to two viewer pages");
  assert.deepEqual([...sectionIds].sort(), [...EXPECTED_SECTION_IDS].sort());
});

test("PRYSM-V2-SECTION-VIEWER-01: rendered report has left sticky navigation and one link per page", () => {
...
test("PRYSM-V2-SECTION-VIEWER-01: hash navigation is deterministic and invalid hashes fall back safely", () => {
  const html = renderReportV2(model());
  assert.match(html, /const fallback = pages\[0\]/);
  assert.match(html, /return byId\.get\(requested\) \|\| fallback/);
  assert.match(html, /history\.replaceState\(null, "", "#" \+ page\.pageId\)/);
  assert.match(html, /link\.setAttribute\("aria-current", "page"\)/);
  assert.match(html, /classList\.toggle\("viewer-active", activeIds\.has\(id\)\)/);
});

test("PRYSM-V2-SECTION-VIEWER-01: current page has browser print/PDF control and print isolation", () => {
  const html = renderReportV2(model());
  assert.match(html, />Print or save this page as PDF<\/button>/);
  assert.match(html, /onclick="window\.print\(\)"/);
  assert.match(html, /body\.viewer-ready main > section:not\(\.viewer-active\)/);
...
test("PRYSM-V2-SECTION-VIEWER-01: all governed section content remains in the single artifact", () => {
  ...
  for (const sectionId of EXPECTED_SECTION_IDS) {
    assert.ok(html.includes(`id="${sectionId}"`), `section ${sectionId} remains rendered`);
  }
  assert.match(html, /data-report-design="2\.0\.0"/);
  assert.match(html, /data-viewer-version="2\.1\.0"/);
});

test("PRYSM-V2-SECTION-VIEWER-01: identical inputs still render byte-identically", () => {
  const m = model();
