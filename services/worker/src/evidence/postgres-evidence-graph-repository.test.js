...
test("retrieval documents fail closed when PostgreSQL lacks pgvector", async () => {
  const repo = setup();
  ...
  await assert.rejects(() => repo.upsertRetrievalDocuments([document]), /vector|type|extension/i);
});
