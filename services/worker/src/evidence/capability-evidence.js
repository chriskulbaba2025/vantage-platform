...
function deriveCapabilities(evidence, auditId, pathValidationEvidence) {
  ...
  // ── content.body ─────────────────────────────────────────────────────
  {
    ...
    const cp = isArray(site?.contentParsing) ? site.contentParsing : null;
    const cpCoverage = coverageFromAcq(cpAcq);
    let status;
    let limitations = [];
    let requiredFields = false;
    if (cpAcq && typeof cpAcq.requested === "number" && cpAcq.requested > 0) {
      if (cpAcq.failed === 0 && cpAcq.completed === cpAcq.requested) {
        // Collected for every requested key page. Main content may still be
        // absent per page — that is a per-page observation, not unknown.
        status = CAPABILITY_STATUS.AVAILABLE;
        requiredFields = true;
      } else if ((cpAcq.completed ?? 0) > 0) {
        status = CAPABILITY_STATUS.PARTIAL;
        requiredFields = true;
      } else {
        status = CAPABILITY_STATUS.UNAVAILABLE;
        limitations.push("Content parsing requested but no page returned usable content");
      }
      ...
      status = CAPABILITY_STATUS.UNAVAILABLE;
      limitations.push("Provider pages endpoint returned no page body content (metadata-only crawl)");
    } else {
      status = CAPABILITY_STATUS.UNAVAILABLE;
      limitations.push("Page body content evidence was not collected");
    }
    caps["content.body"] = capability({
      capability: "content.body", status, coverage: cpCoverage, provenance: siteProv,
      limitations, requiredFieldsPresent: requiredFields,
    });
  }
  ...
  // ── technical.indexability ───────────────────────────────────────────
  {
    const niAcq = acquisitionOf(site, "nonIndexable");
    const niCoverage = coverageFromAcq(niAcq);
    let status;
    const limitations = [];
    let requiredFields = false;
    if (niAcq && typeof niAcq.requested === "number" && niAcq.requested > 0) {
      if ((niAcq.failed ?? 0) === 0) {
        status = CAPABILITY_STATUS.AVAILABLE;
        requiredFields = true;
      } else {
        status = CAPABILITY_STATUS.PARTIAL;
        limitations.push("Non-indexable retrieval was partially unsuccessful");
      }
    } else if (isArray(site?.pages) && site.pages.some((p) => typeof p?.indexable === "boolean")) {
      status = CAPABILITY_STATUS.PARTIAL;
      limitations.push("Page-level indexable flags only — non-indexable endpoint not collected");
    } else {
      status = CAPABILITY_STATUS.UNAVAILABLE;
      limitations.push("Indexability evidence was not collected");
    }
    caps["technical.indexability"] = capability({
      capability: "technical.indexability", status, coverage: niCoverage,
      provenance: siteProv, limitations, requiredFieldsPresent: requiredFields,
    });
  }

  // ── technical.redirects ──────────────────────────────────────────────
  {
    const rcAcq = acquisitionOf(site, "redirectChains");
    const rcCoverage = coverageFromAcq(rcAcq);
    let status;
    const limitations = [];
    let requiredFields = false;
    if (rcAcq && typeof rcAcq.requested === "number" && rcAcq.requested > 0) {
      if ((rcAcq.failed ?? 0) === 0) {
        status = CAPABILITY_STATUS.AVAILABLE;
        requiredFields = true;
      } else if ((rcAcq.completed ?? 0) > 0) {
        status = CAPABILITY_STATUS.PARTIAL;
        limitations.push("Redirect-chain checks partially unsuccessful");
      } else {
        status = CAPABILITY_STATUS.UNAVAILABLE;
        limitations.push("Redirect-chain retrieval returned no usable data");
      }
    } else if (isArray(site?.pages) && site.pages.some((p) => p?.redirectDestination)) {
      status = CAPABILITY_STATUS.PARTIAL;
      limitations.push("Page-level redirect destinations only — redirect-chain endpoint not collected");
    } else {
      status = CAPABILITY_STATUS.UNAVAILABLE;
      limitations.push("Redirect evidence was not collected");
    }
    caps["technical.redirects"] = capability({
      capability: "technical.redirects", status, coverage: rcCoverage,
      provenance: siteProv, limitations, requiredFieldsPresent: requiredFields,
    });
  }
  ...
  // ── technical.resources ──────────────────────────────────────────────
  {
    const resAcq = acquisitionOf(site, "resources");
    const resCoverage = coverageFromAcq(resAcq);
    let status;
    const limitations = [];
    let requiredFields = false;
    if (resAcq && typeof resAcq.requested === "number" && resAcq.requested > 0) {
      if ((resAcq.failed ?? 0) === 0) {
        status = CAPABILITY_STATUS.AVAILABLE;
        requiredFields = true;
      } else if ((resAcq.completed ?? 0) > 0) {
        status = CAPABILITY_STATUS.PARTIAL;
        limitations.push("Resource checks partially unsuccessful");
      } else {
        status = CAPABILITY_STATUS.UNAVAILABLE;
        limitations.push("Resource retrieval returned no usable data");
      }
    } else {
      status = CAPABILITY_STATUS.UNAVAILABLE;
      limitations.push("Resource evidence was not collected");
    }
    caps["technical.resources"] = capability({
      capability: "technical.resources", status, coverage: resCoverage,
      provenance: siteProv, limitations, requiredFieldsPresent: requiredFields,
    });
  }
  ...
  // ── technical.headers ────────────────────────────────────────────────
  {
    ...
    caps["technical.headers"] = capability({
      ...
      status: headersAvailable ? CAPABILITY_STATUS.AVAILABLE : CAPABILITY_STATUS.UNAVAILABLE,
      coverage: { requested: null, completed: null, failed: null },
      provenance: siteProv,
  ...
  // ── schema.structured_data ───────────────────────────────────────────
  {
    const schemaTypes = isArray(site?.schemaTypes) ? site.schemaTypes : null;
    const microdataTypes = isArray(site?.microdataTypes) ? site.microdataTypes : null;
    const mdAcq = acquisitionOf(site, "microdata");
    let status;
    const limitations = [];
    let requiredFields = false;
    if ((schemaTypes && schemaTypes.length > 0) || (microdataTypes && microdataTypes.length > 0)) {
      status = CAPABILITY_STATUS.AVAILABLE;
      requiredFields = true;
    } else if (contentState === "available") {
      ...
      status = CAPABILITY_STATUS.AVAILABLE;
      requiredFields = true;
    } else if (mdAcq && (mdAcq.completed ?? 0) > 0) {
      // Provider micromarkup validation ran and returned nothing — confirmed absence.
      status = CAPABILITY_STATUS.AVAILABLE;
      requiredFields = true;
    } else {
      status = CAPABILITY_STATUS.UNAVAILABLE;
      limitations.push("Structured-data evidence was not collected");
    }
    caps["schema.structured_data"] = capability({
      capability: "schema.structured_data", status,
      coverage: coverageFromAcq(mdAcq), provenance: siteProv,
      limitations, requiredFieldsPresent: requiredFields,
    });
  }
  ...
  {
    const perfStatus = performance?.sourceStatus ?? null;
    const perfProv = {
      source: performance?.provider || PERF_SOURCE,
      adapterVersion: performance?.adapterVersion ?? null,
      artifactRef: null,
    };
    const perfCoverage = performance?.coverage
      ? {
          requested: performance.coverage.requested ?? null,
          completed: performance.coverage.completed ?? null,
          failed: performance.coverage.failed ?? null,
        }
      : { requested: null, completed: null, failed: null };
    ...
    if (perfStatus === "AVAILABLE") {
      ...
      labLimitations.push("Lab performance evidence was not collected");
    }
    caps["performance.lab"] = capability({
      capability: "performance.lab", status: labStatus, coverage: perfCoverage,
      provenance: perfProv, limitations: labLimitations,
      requiredFieldsPresent: labStatus === "AVAILABLE" || labStatus === "PARTIAL",
    });

    const fieldData = performance?.fieldData;
    const hasFieldData = fieldData && typeof fieldData === "object" &&
  ...
  for (const name of CAPABILITIES) {
    ...
    switch (cap.status) {
      ...
      case CAPABILITY_STATUS.NOT_APPLICABLE:
        ...
        break;
      default:
        throw new Error(`Capability ${name} has invalid status: ${cap.status}`);
    }
  }

  return {
    contractVersion: "1.0.0",
    capabilityEvidenceVersion: CAPABILITY_EVIDENCE_VERSION,
    auditId,
    generatedAt,
    capabilities,
    summary,
 ...
 */
export async function persistCapabilityEvidence({ store, scope, evidence, validateContract }) {
  if (validateContract) {
    ...
    if (!sv || !sv.valid) {
      throw new Error(
        `Capability evidence validation failed: ${JSON.stringify((sv?.errors || []).slice(0, 5))}`,
      );
    }
  }
  ...
  if (!stored || stored.length !== bytes.length) {
    throw new Error("Capability evidence read-back byte mismatch");
  }
  if (record.sha256 !== sha256(bytes)) {
    throw new Error("Capability evidence SHA-256 mismatch");
  }
  if (typeof store.verify === "function") {
    const verified = await store.verify(record);
    if (!verified) throw new Error("Capability evidence store.verify() failed");
  }
 ...
 */
export async function loadAndValidateCapabilityEvidence({ store, scope, validateContract }) {
  ...
  if (!bytes || bytes.length === 0) {
    throw new Error("Capability evidence artifact not found or empty — cannot proceed without governed capability evidence");
  }
  ...
  try {
    ...
    throw new Error(`Capability evidence artifact is not valid JSON: ${parseErr.message}`);
  }
  ...
  if (validateContract) {
    ...
    if (!sv || !sv.valid) {
      throw new Error(
        `Capability evidence validation failed on load: ${JSON.stringify((sv?.errors || []).slice(0, 5))}`,
      );
    }
  }
