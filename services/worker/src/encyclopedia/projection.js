...
export function projectFinding(finding, { ruleMap = RULE_TO_CANONICAL } = {}) {
  ...
  const projected = {
    ...
    conversionAction: finding?.conversionAction || finding?.primaryConversionAction || null,
    buyerDecisionQuestion: finding?.buyerDecisionQuestion || null,
    mechanismFamily: finding?.mechanismFamily || null,
    dependsOnFindingIds: Array.isArray(finding?.dependsOnFindingIds) ? [...finding.dependsOnFindingIds] : [],
    dependsOnCanonicalProblemIds: Array.isArray(finding?.dependsOnCanonicalProblemIds) ? [...finding.dependsOnCanonicalProblemIds] : [],
