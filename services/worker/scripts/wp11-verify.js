...
function run(cmd, label) {
  try {
    execSync(cmd, { stdio: "pipe", encoding: "utf-8", timeout: 300_000 });
    pass(label);
    return true;
  } catch (err) {
...
console.log("WP11 Full Verification\n=====================");
...
// WP11 acceptance
run("npm run acceptance:wp11", "WP11 acceptance");

// Full regression
run("npm test", "Full regression");

// Scope check
run("npm run wp11:scope-check", "Scope check");

console.log("\n" + (failures > 0 ? failures + " verification step(s) failed." : "WP11 Full Verification PASS."));
